    ...
    else
    {
      ...
      if (_GetRxStallStatus(Related_Endpoint))
      {
        if (Related_Endpoint == ENDP0)
        {
          ...
          _SetEPRxCount(Related_Endpoint, Device_Property.MaxPacketSize);
...
void DataStageOut(void)
{
  ...
  if (pEPinfo->CopyData && save_rLength)
  {
    ...
    PMAToUserBufferCopyENDP0(Buffer, Length);
  }
...
void DataStageIn(void)
{
  ...
  UserToPMABufferCopyENDP0(DataBuffer, Length);
...
void NoData_Setup0(void)
{
	...
	if (Type_Recipient == (STANDARD_REQUEST | DEVICE_RECIPIENT))
	{
		...
		else if (RequestNo == SET_ADDRESS)	// SET ADDRESS
		{
			if ((pInformation->USBwValue0 > 127) || (pInformation->USBwValue1 != 0)
			    ...
			    || (pInformation->Current_Configuration != 0))	// 设备地址必须<=127
			{
	...
	{
		if (RequestNo == SET_INTERFACE)	// SET INTERFACE
		{
	...
	{
		if (RequestNo == CLEAR_FEATURE)		// CLEAR FEATURE for EndPoint
		{
			Result = Standard_ClearFeature();
		}
		else if (RequestNo == SET_FEATURE)	// SET FEATURE for EndPoint
		{
...
uint8_t Setup0_Process(void)
{
	...
	if (pInformation->ControlState != PAUSE)
	{
		...
		pBuf.w++;
		bTemp = (uint8_t)(*pBuf.w & 0xff);
		pInformation->USBwValue = (*pBuf.w >> 8) | ((uint16_t)bTemp << 8);;
		pBuf.w++;
		pBuf.w++;
		bTemp = (uint8_t)(*pBuf.w & 0xff);
		pInformation->USBwIndex = (*pBuf.w >> 8) | ((uint16_t)bTemp << 8);;
		pBuf.w++;
		pBuf.w++;
...
uint8_t In0_Process(void)
{
	...
	else if (ControlState == WAIT_STATUS_IN)
	{
		    ...
		    (Type_Recipient == (STANDARD_REQUEST | DEVICE_RECIPIENT)))
		{
			...
			_SetDADDR(pInformation->USBwValue0 | DADDR_EF);
...
uint8_t Out0_Process(void)
{
	...
	{
		ControlState = STALLED;	// host aborts the transfer before finish
	}
