...
void MASS_init()
{
  ...
  _SetISTR(0);
  wInterrupt_Mask = (CNTR_CTRM  | CNTR_RESETM);
  /* set interrupts mask */
  _SetCNTR(wInterrupt_Mask);

  bUSBDeviceState = UNCONNECTED;
}
...
void MASS_Reset()
{
  ...
  _SetBTABLE(0x00);	//#define BTABLE_ADDRESS      (0x00)
  ...
  UserToPMABufferCopyENDP1(0,1);	//主机第一次会读走这条信息
  ...
  _Clear_Status_Out(ENDP2);
  _SetEPRxCount(ENDP2, Device_Property.MaxPacketSize);
